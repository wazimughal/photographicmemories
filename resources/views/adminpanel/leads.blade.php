...
@push('title')
    <title>Leads| {{ config('constants.app_name') }}</title>
@endpush
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-2"><a style="width:60%" href="{{url('/admin/leads/add')}}" class="btn btn-block btn-success btn-lg">Add New <i class="fa fa-plus"></i></a></div>
                            ...
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <?php 
                                            ...
                                            <tr id="row_{{$data['id']}}">
                                                ...
                                                <td id="status{{ $data['id'] }}">
                                                    ...
                                                    <a @disabled(true) class="btn btn-danger btn-flat btn-sm"><i class="fas fa-chart-line"></i> Pending</a>
                                                    @elseif ($data['status']==config('constants.lead_status.approved'))
                                                    <a @disabled(true) class="btn bg-gradient-success btn-flat btn-sm"><i class="fas fa-chart-line"></i> Approved</a>
                                                    @else
                                                    <a @disabled(true) class="btn bg-gradient-secondary btn-flat btn-sm"><i class="fas fa-chart-line"></i> Cancelled</a>
                                                ...
                                                <td id="is_active_{{ $data['id'] }}">
                                                    ...
                                                    @endif --}}
                                                    <select datacounter="{{$counter}}" dataid="{{ $data['id'] }}" class="form-control select2bs4 current_status" style="width: 100%;">
                                                        <option datacounter="{{$counter}}" dataid="{{ $data['id'] }}" value="{{config('constants.lead_status.pending')}}" @php if(config('constants.lead_status.pending')==$data['status']){echo 'selected="selected"';}  @endphp>Pending</option>
                                                        <option datacounter="{{$counter}}" dataid="{{ $data['id'] }}" value="{{config('constants.lead_status.approved')}}"  @php if(config('constants.lead_status.approved')==$data['status']){echo 'selected="selected"';}  @endphp>Approve</option>
                                                        <option datacounter="{{$counter}}" dataid="{{ $data['id'] }}" value="{{config('constants.lead_status.cancelled')}}" @php if(config('constants.lead_status.cancelled')==$data['status']){echo 'selected="selected"';}  @endphp>Cancel</option>
                                                        </select>
                                                </td>
                                                <td>
                                               
                                                        <button onClick="updateForm({{ $data['id'] }},{{ $counter }})" class="btn btn-info btn-block btn-sm"><i
                                                            class="fas fa-edit"></i> Edit</button>
                                                        <button onClick="viewLeadData({{ $data['id'] }},{{ $counter }})" class="btn btn-primary btn-block btn-sm"><i
                                    ...
                                    <tfoot>
                                        ...
                                        <tr><td colspan="8"><div class="text-right"> {{$leadsData->links()}}</div></td></tr>
                                    </tfoot>
                                </table>
                                {{-- Pagination --}}
