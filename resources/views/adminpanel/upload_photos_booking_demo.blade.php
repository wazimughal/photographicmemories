...
@section('main-section')
                                                ...
                                                <form id="change_event_status" action="" method="POST">
                                                    ...
                                                    <div class="form-group clearfix">
                                                        <div class="icheck-success d-inline">
                                                          <input type="radio" {{($bookingData['is_active']==1)?'checked':''}}  onclick="do_action({{$bookingData['id']}},'change_event_status','active_event2')" value="{{base64_encode(1)}}" name="active_event"  id="active_event1">
                                                          <label for="active_event1"> Activate
                                                          </label>
                                                        </div>
                                                        <div class="icheck-success d-inline">
                                                          <input type="radio" {{($bookingData['is_active']==3)?'checked':''}}  onclick="do_action({{$bookingData['id']}},'change_event_status','active_event1')" value="{{base64_encode(3)}}" name="active_event" id="active_event2">
                                                   ...
                                                   if(count($bookingData['gallery'])>0)
                                                      foreach($bookingData['gallery'] as $data){
                                                        ...
                                                        <div id="file_{{$data['id']}}" class="col-3 text-center" style="position: relative;">
                                                          ...
                                                          @endif
                                                          <a href="{{$data['path']}}" target="_blank"><img class="w-100 shadow-1-strong rounded mb-4 img-thumbnail" src="{{$thumb_img}}" width="200" alt="Uploaded Image"></a><br>
                                                          <div class="btn btn-flat btn-info align-center" onclick="open_template_editor('{{$data['path']}}')">Edit</div>
                                                        </div>
	...
	$booking_id = $id;
?>
                                                    <form action="{{route('booking.add_photos.demo',array('id'=>$id))}}"
                                                        method="post" enctype="multipart/form-data" id="datanodeupload"
                                                        class="dropzone ">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="tab-content">
                                                    <form method="post" id="submit_photographer_comment" >
                                                        ...
                                                        <input type="hidden" name="action" value="submit_photographer_comment">
                                                        <input type="hidden" name="slug" value="{{$userData['get_groups']['slug']}}">
                                                        <input type="hidden" name="for_section" value="photographer_section">
...
@section('head-js-css')
    ...
    <style>
    .pixo-modal{z-index:99999 !important;}
        </style>
...
if (confirm('Are you sure? you want to delete this file?')) {
    ...
    $.ajax({
        ...
        function do_action(id, action_name = '', element_id='') {
            ...
            if(action_name=='change_event_status'){
