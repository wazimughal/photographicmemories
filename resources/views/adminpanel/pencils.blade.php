        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-2"><a style="width:60%" href="{{ route('pencils.pencils_form') }}"
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="row_{{ $pencil['id'] }}">
                                            ...
                                            <td id="venue_group_{{ $pencil['id'] }}">
                                                @if (isset($pencil['venue_group']) && isset($pencil['venue_group']['userinfo'][0]['vg_name']))
                                            ...
                                            <td id="customer_name_{{ $pencil['id'] }}">
                                                {{ isset($pencil['customer']['userinfo'][0]['name']) ? $pencil['customer']['userinfo'][0]['name'] : '' }}
                                            </td>
                                            ...
                                            <td id="pencil_by_{{ $pencil['id'] }}">{{ booking_status($pencil['status']) }}
                                            </td>

                                            <td>
                                                @if ($user->group_id == config('constants.groups.admin'))
                                                    <a href="{{ route('bookings.bookings_form', $pencil['id']) }}"
                                                        class="btn btn-success btn-block btn-sm"><i class="fas fa-plus"></i>
                                                        Booking</a>
                                                @endif
                                                <a href="{{ route('pencil.view', $pencil['id']) }}"
                                                ...
                                                @elseif ($pencil['is_active'] == 2)
                                                    <button onClick="do_action({{ $pencil['id'] }},'restore_booking')"
    ...
    <script>
        ...
        function do_action(id, action_name = '') {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.error == 'No') {
                        if (action_name == 'delete_pencil' || action_name == 'trash_booking' || action_name ==
                            'restore_booking')
                            $('#row_' + id).remove();
                        $(document).Toasts('create', {
                            class: 'bg-success',
