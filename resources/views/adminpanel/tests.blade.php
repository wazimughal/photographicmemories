...
@push('title')
    <title>Lab Tests| {{ config('constants.app_name') }}</title>
@endpush
                    ...
                    <div class="col-12">
                        ...
                        <div class="card card-success">
                            <div class="card-header">
                                <h3 class="card-title">LAB TESTS DETAIL</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @php
                                            $counter = 1;
                                        @endphp
                                        @foreach ($LabTestsData as $labTests)
                                            @php
                                                    $testParamsName = implode(', ', array_column($labTests['get_params'], 'parameter_name'));
                                            @endphp
                                            <tr>
                                                <td><strong>{{ $labTests['test_name'] }}</strong></td>
                                                <td>{{ $labTests['description'] }}</td>
                                                <td>{{ $testParamsName }}</td>
                                                <td><a href="{{url('/admin/lab-tests/edit').'/'.$labTests['id']}}" class="btn btn-info btn-block btn-sm"><i class="fas fa-edit"></i>
                                                        Edit</a><a href="{{url('/admin/lab-tests/delete').'/'.$labTests['id']}}" class="btn btn-danger btn-block btn-sm">
